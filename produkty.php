






<div class="produktyBox" id="produktyBox">
    <div class="produkt" onclick="zobraz('seznamProduktu', 'Broušené sklo')">

        <h4 class="produktText">
            Broušené sklo 
        </h4>
        <img class="produktimg" src="./img/foto/buildings.jpg" >

    </div>
    <div class="produkt produktR" onclick="zobraz('seznamProduktu', 'Skleněná světla')">
        <h4 class="produktText">
            Skleněná světla
        </h4>
        <img class="produktimg " src="./img/foto/drink.jpg" >

    </div>
    <div class="produkt produktR" onclick="zobraz('seznamProduktu', 'Nápojové sklo')">
        <h4 class="produktText">
            Nápojové sklo
        </h4>
        <img class="produktimg" src="./img/foto/forest.jpg">
    </div>
    <div class="produkt produktR" onclick="zobraz('seznamProduktu', 'Vázy z plochého skla')">
        <h4 class="produktText">
            Vázy z plochého skla
        </h4>
        <img class="produktimg" src="./img/foto/cerem.jpg">
    </div>
    <div class="produkt produktR" onclick="zobraz('seznamProduktu', 'Dekorativní sklo')">
        <h4 class="produktText">
            Dekorativní sklo
        </h4>
        <img class="produktimg" src="./img/foto/cerem.jpg">
    </div>
    <div class="produkt produktR" onclick="zobraz('seznamProduktu', 'Šperky')">
        <h4 class="produktText">
            Šperky
        </h4>
        <img class="produktimg" src="./img/foto/cerem.jpg">
    </div>
    
</div>











